...
function allLayerClose(idx,length,flag) {
 ...
 for(var i = 1; i < length+1; i++) {
	...
	<form class="navbar-form navbar-left" role="search"
		...
		<c:choose>
			<c:when test="${sessionScope.loginVo!=null}">
			<a class="btn btn-default" href="${initParam.root}write.do?type=board_community">글쓰기</a>
			</c:when>
			<c:otherwise>
			&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
			</c:otherwise>
...
<div class="col-md-12 text-center">
	<ul class="pagination">
		<c:if test="${pb.previousPageGroup}">
			<li><a
				href="searchPosting.do?pageNo=${pb.startPageOfPageGroup-1}&type=board_community&word=${requestScope.lvo.word}&category=${requestScope.lvo.category}">Prev</a></li>
		</c:if>
		<c:forEach var="i" begin="${pb.startPageOfPageGroup}"
			end="${pb.endPageOfPageGroup}">
			<c:choose>
				<c:when test="${pb.nowPage!=i}">
					<li><a href="searchPosting.do?pageNo=${i}&type=board_community&word=${requestScope.lvo.word}&category=${requestScope.lvo.category}">${i}</a></li>
				</c:when>
				<c:otherwise>
	...
	&nbsp;
		...
		<c:if test="${pb.nextPageGroup}">
			<li><a href="searchPosting.do?pageNo=${pb.endPageOfPageGroup+1}&type=board_community&word=${requestScope.lvo.word}&category=${requestScope.lvo.category}">Next</a></li>
		</c:if>
	</ul>
</div>
