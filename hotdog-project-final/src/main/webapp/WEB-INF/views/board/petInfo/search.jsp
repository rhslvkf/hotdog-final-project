        ...
        <table class="table table-striped custab">
          ...
          <tbody>
			<tr>
				...
				<a href="${initParam.root}showContent.do?no=${bvo.boardNumber }&type=board_petInfo">
				${bvo.boardTitle }</a>
				</c:when>
				<c:otherwise>
				${bvo.boardTitle}
...
<!-- 				/////////////////////////////////////////////////////////////////////////////////////////////////// -->
				...
				<td>${bvo.boardHits }</td>
							...
							<c:if test="${bvo.memberVO.memberId==sessionScope.loginVo.memberId || sessionScope.loginVo.memberGrade=='ADMIN'}">
								<a class="btn btn-info btn-xs"
									href="updateView.do?no=${bvo.boardNumber}&type=board_petInfo" onclick="return confirm('수정하시겠습니까?')">
									<span class="glyphicon glyphicon-edit"></span>
									 수정
								</a>
								<a
									href="auth_deletePosting.do?no=${bvo.boardNumber }&type=board_petInfo" onclick="return confirm('삭제하시겠습니까?')"
	...
	<form class="navbar-form navbar-left" role="search"
		...
		<input type="hidden" name="type" value="board_petInfo">
		<div class="form-group">
		<c:choose>
			<c:when test="${sessionScope.loginVo!=null && sessionScope.loginVo.memberGrade !='BRONZE'}">
			<a class="btn btn-default" href="${initParam.root}auth_write.do?type=board_petInfo">글쓰기</a>
			</c:when>
			<c:otherwise>
			&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
			</c:otherwise>
...
<div class="col-md-12 text-center">
	<ul class="pagination">
		<c:if test="${pb.previousPageGroup}">
			<li><a
				href="searchPosting.do?pageNo=${pb.startPageOfPageGroup-1}&type=board_petInfo&word=${requestScope.lvo.word}&category=${requestScope.lvo.category}">Prev</a></li>
		</c:if>
		<c:forEach var="i" begin="${pb.startPageOfPageGroup}"
			end="${pb.endPageOfPageGroup}">
			<c:choose>
				<c:when test="${pb.nowPage!=i}">
					<li><a href="searchPosting.do?pageNo=${i}&type=board_petInfo&word=${requestScope.lvo.word}&category=${requestScope.lvo.category}">${i}</a></li>
				</c:when>
				<c:otherwise>
	...
	&nbsp;
		...
		<c:if test="${pb.nextPageGroup}">
			<li><a href="searchPosting.do?pageNo=${pb.endPageOfPageGroup+1}&type=board_petInfo&word=${requestScope.lvo.word}&category=${requestScope.lvo.category}">Next</a></li>
		</c:if>
	</ul>
</div>
