...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
    ...
    $(document).ready(function(){
    	$("#listImg").click(function(){    		
    		location.href = "${initParam.root}getPostingList.do?type=board_petInfo";
    	});
    	$("#deleteImg").click(function(){ 
    		if(confirm("게시물을 삭제하시겠습니까?"))
    			location.href="auth_deletePosting.do?no=${requestScope.bvo.boardNumber}&type=board_petInfo";
    	});
    	...
    	$("#scrapeImg").click(function(){  
    		if(confirm("게시물을 스크랩하시겠습니까?")){
    			$.ajax({
					type:"POST",
					url:"scrape.do",
					data:"no=${requestScope.bvo.boardNumber}&type=board_petInfo",
  	...
  	$(document).ready(function() {
  		...
  		var comment="";
			$.ajax({
				type : "post",
				url : "commentList.do",
				data : "commentBoardNumber=${requestScope.bvo.boardNumber }&type=board_petInfo",
				dataType : "json",
				success : function(data) {
	...
	$("#commentParentSubmit").click(function(){
			...
			if($(":input[name=PcommentContent]").val() == ""){
				...
				$.ajax({
						...
						data : $("#commentForm").serialize()+"&commentBoardNumber=${requestScope.bvo.boardNumber }"+"&type=board_petInfo",
						dataType :"json",
						success : function(data) {
		  ...
		  if($(this).attr("name")=="pDel"||$(this).attr("name") == "cDel"){
					if (confirm("정말 삭제하시겠습니까??") == true) { //확인
						...
						$.ajax({
							...
							data : "commentNumber="+cno+"&type=board_petInfo",
						});
					 if($('#commentTable').contents().size()!=0)
  ...
  $(document).on("click","#commentChildSubmit", function(){
    ...
    $("#commentTable td").remove();  
		   	...
		   	$.ajax({
				type : "post",
				url : "commentReply.do",
				data : "memberNickName="+cName.val()+"&commentContent="+cText.val()+ "&commentGroup="+cgroup.val()+ "&commentBoardNumber=${requestScope.bvo.boardNumber }"+"&commentDate='0'&type=board_petInfo",
				dataType : "json",
				success : function(data) {
					 $.each(data, function(i, value) {
