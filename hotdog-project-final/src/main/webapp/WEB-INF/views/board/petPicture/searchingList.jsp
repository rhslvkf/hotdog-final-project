...
<div class="col-md-12 text-center">
	<ul class="pagination">
		<c:if test="${pb.previousPageGroup}">
			<li><a
				href="searchBoardPosting.do?pageNo=${pb.startPageOfPageGroup-1}&type=board_petPicture&word=${requestScope.lvo.word}">Prev</a></li>
		</c:if>
		<c:forEach var="i" begin="${pb.startPageOfPageGroup}"
			end="${pb.endPageOfPageGroup}">
			<c:choose>
				<c:when test="${pb.nowPage!=i}">
					<li><a href="searchBoardPosting.do?pageNo=${i}&type=board_petPicture&word=${requestScope.lvo.word}">${i}</a></li>
				</c:when>
				<c:otherwise>
	...
	&nbsp;
		...
		<c:if test="${pb.nextPageGroup}">
			<li><a href="searchBoardPosting.do?pageNo=${pb.endPageOfPageGroup+1}&type=board_petPicture&word=${requestScope.lvo.word}">Next</a></li>
		</c:if>
	</ul>
</div>
