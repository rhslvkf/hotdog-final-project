...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
    ...
    $(document).ready(function(){
    	$("#listImg").click(function(){    		
    		location.href = "${initParam.root}getPostingList.do?type=board_petPicture";
    	});
    	$("#deleteImg").click(function(){ 
    		if(confirm("게시물을 삭제하시겠습니까?"))
    			location.href="auth_deletePosting.do?no=${requestScope.bvo.boardNumber}&type=board_petPicture";
    	});
  	...
  	$(document).ready(function() {
  		...
  		var comment="";
			$.ajax({
				type : "post",
				url : "commentList.do",
				data : "commentBoardNumber=${requestScope.bvo.boardNumber }&type=board_petPicture",
				dataType : "json",
				success : function(data) {
	...
	$("#commentParentSubmit").click(function(){
			...
			if($(":input[name=PcommentContent]").val() == ""){
				...
				$.ajax({
						...
						data : $("#commentForm").serialize()+"&commentBoardNumber=${requestScope.bvo.boardNumber }"+"&type=board_petPicture",
						dataType :"json",
						success : function(data) {
								cno=data.cno;
		  ...
		  if($(this).attr("name")=="pDel"||$(this).attr("name") == "cDel"){
					if (confirm("정말 삭제하시겠습니까??") == true) { //확인
						...
						$.ajax({
							...
							data : "commentNumber="+cno+"&type=board_petPicture",
						});
					 if($('#commentTable').contents().size()!=0)
  ...
  $(document).on("click","#commentChildSubmit", function(){
    ...
    $("#commentTable td").remove();  
		   	...
		   	$.ajax({
				type : "post",
				url : "commentReply.do",
				data : "memberNickName="+cName.val()+"&commentContent="+cText.val()+ "&commentGroup="+cgroup.val()+ "&commentBoardNumber=${requestScope.bvo.boardNumber }"+"&commentDate='0'&type=board_petPicture",
				dataType : "json",
				success : function(data) {
					 $.each(data, function(i, value) {
			...
			<td colspan="3"><pre style="white-space: pre-wrap;">${requestScope.bvo.boardContent}
                ...
                <div class="col-md-12 text-center">
                  <span class="pull-center">
                                    <a href="${initParam.root}getPostingList.do?type=board_petPicture" 
								 ...
								 <c:if test="${requestScope.bvo.memberVO.memberId==sessionScope.loginVo.memberId || sessionScope.loginVo.memberGrade=='ADMIN'}">
								    <a href="updateView.do?no=${requestScope.bvo.boardNumber}&type=board_petPicture" onclick="return confirm('수정하시겠습니까?')"
									class="btn btn-lg btn-warning"> 수정</a>
                					<a href="auth_deletePosting.do?no=${requestScope.bvo.boardNumber}&type=board_petPicture" onclick="return confirm('삭제하시겠습니까?')"
